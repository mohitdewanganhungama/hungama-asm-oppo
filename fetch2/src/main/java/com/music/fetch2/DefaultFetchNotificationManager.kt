...
import com.hungama.fetch2.DownloadNotification.ActionType.*
import com.hungama.fetch2.util.DEFAULT_NOTIFICATION_TIMEOUT_AFTER
import com.hungama.fetch2.util.DEFAULT_NOTIFICATION_TIMEOUT_AFTER_RESET
import com.hungama.fetch2.util.onDownloadNotificationActionTriggered
...
abstract class DefaultFetchNotificationManager(context: Context) : FetchNotificationManager {
    ...
    override fun updateNotification(notificationBuilder: NotificationCompat.Builder,
        ...
        if (contentIntent != null){
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                contentPendingIntent = PendingIntent.getActivity(context, 0, contentIntent, PendingIntent.FLAG_IMMUTABLE)
            }else{
    ...
    override fun getActionPendingIntent(downloadNotification: DownloadNotification,
        ...
        synchronized(downloadNotificationsMap) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                pendingIntent = PendingIntent.getBroadcast(context, downloadNotification.notificationId + action, intent, PendingIntent.FLAG_IMMUTABLE)
            }else{
                pendingIntent = PendingIntent.getBroadcast(context, downloadNotification.notificationId + action, intent, PendingIntent.FLAG_UPDATE_CURRENT)
            }
    ...
    override fun getGroupActionPendingIntent(groupId: Int,
        ...
        synchronized(downloadNotificationsMap) {
            ...
            var pendingIntent:PendingIntent? = null
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                pendingIntent = PendingIntent.getBroadcast(context, groupId + action, intent, PendingIntent.FLAG_IMMUTABLE)
            }else{
                pendingIntent = PendingIntent.getBroadcast(context, groupId + action, intent, PendingIntent.FLAG_UPDATE_CURRENT)
            }
            return pendingIntent
        }
    }
...
//                    notificationId = downloadNotification.notificationId
    ...
    override fun postDownloadUpdate(download: Download): Boolean {
        return synchronized(downloadNotificationsMap) {
            ...
            //downloadNotification.title = getDownloadNotificationTitle(download.extras.getString("contentTitle", "--"))
            downloadNotification.title = download.extras.getString("contentTitle", "--")
