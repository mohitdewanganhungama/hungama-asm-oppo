...
class AlbumArtContentProvider : ContentProvider() {
    ...
    companion object {
        ...
        fun mapUri(uri: Uri): Uri {
            ...
            Log.d("AlbumArtContentProvider", "mapUri:${uri} path:${path}")
            val contentUri = Uri.Builder()
                .scheme(ContentResolver.SCHEME_CONTENT)
                .authority("com.hungama.myplay.activity")
                .path(path)
                .build()
            uriMap[contentUri] = uri

            Log.d("AlbumArtContentProvider", "contentUri:${contentUri} mapUri:${uri} path:${path}")
            return contentUri
        }
    }
