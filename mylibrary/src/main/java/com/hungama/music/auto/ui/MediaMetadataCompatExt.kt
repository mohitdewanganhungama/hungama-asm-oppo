...
import com.hungama.music.auto.media.extensions.toUri
import com.hungama.music.auto.media.library.JsonSource
...
//        setWriter(writer)
        ...
        extras.putLong(MediaMetadataCompat.METADATA_KEY_DURATION, 161L)
        setExtras(extras)
    }.build()
}

@OptIn(UnstableApi::class) fun MediaMetadataCompat.toMediaItem(): MediaItem {
    Log.d("TAG", "toMediaItem:mediaUri:${mediaUri} ")
    var mimType = MimeTypes.BASE_TYPE_AUDIO
    if (mediaUri.toString().contains(".m3u8", true)!!) {
        mimType = MimeTypes.APPLICATION_M3U8
    } else if (mediaUri.toString().contains(".mp3", true)!!) {
        mimType = MimeTypes.BASE_TYPE_AUDIO
    } else if (mediaUri.toString().contains(".mpd", true)!!){
        mimType = MimeTypes.APPLICATION_MPD
    } else if (mediaUri.toString().contains(".mp4", true)!!) {
