...
public class BillingProcessor extends BillingBase
{
	...
	private class HistoryInitializationTask extends AsyncTask<Void, Void, Boolean>
	{
		...
		protected void onPostExecute(Boolean restored)
		{
			...
			CommonUtils.INSTANCE.setLog("HistoryTask", "onPostExecute restored"+restored);
	...
	{
		CommonUtils.INSTANCE.setLog(LOG_TAG, "initialize called");
			...
			billingService.startConnection(new BillingClientStateListener()
			{
				...
				public void onBillingSetupFinished(@NonNull BillingResult billingResult)
				{
					if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK)
					{
						...
						CommonUtils.INSTANCE.setLog(LOG_TAG, " GooglePlayConnection IsConnected");
					...
					{
						CommonUtils.INSTANCE.setLog(LOG_TAG, " retryBillingClientConnection");
				...
				{
					CommonUtils.INSTANCE.setLog(LOG_TAG, " ServiceDisconnected BillingServiceDisconnected, trying new Connection");
	...
	**/
	public void release()
	{
		if (isConnected())
		{
			CommonUtils.INSTANCE.setLog(LOG_TAG, "BillingClient can only be used once -- closing connection");
					...
					for (Purchase purchaseItem : list)
					{
						...
						if (!TextUtils.isEmpty(jsonData))
						{
							...
							catch (Exception e)
							{
								...
								CommonUtils.INSTANCE.setLog("Error in loadPurchasesByType", e.getMessage().toString());
								reportPurchasesError(listener);
							}
						}
					}
	...
	{
		if (productId != null && !isSubscriptionUpdateSupported())
		{
			return false;
		}
	...
	private boolean purchase(final Activity activity, final String oldProductId, final String productId,
							 String purchaseType)
	{
		CommonUtils.INSTANCE.setLog(LOG_TAG, "purchase oldProductId"+oldProductId+" productId"+productId +" purchaseType"+purchaseType);
...
//													  .build();
			CommonUtils.INSTANCE.setLog(LOG_TAG, "SkuDetailsParams params"+params);
			...
			billingService.queryProductDetailsAsync(
					...
					{
						@Override
						public void onProductDetailsResponse(@NonNull BillingResult billingResult,  List<ProductDetails> skuList) {
							CommonUtils.INSTANCE.setLog(LOG_TAG, "onSkuDetailsResponse called");
							if (skuList != null && !skuList.isEmpty())
							{
								CommonUtils.INSTANCE.setLog(LOG_TAG, "onSkuDetailsResponse called"+skuList.get(0));
								startPurchaseFlow(activity, skuList.get(0), productId);

							}
							else
							{
								// This will occur if product id does not match with the product type
								CommonUtils.INSTANCE.setLog("onSkuResponse: ", "product id mismatch with Product type");
								reportBillingError(
		...
		{
			CommonUtils.INSTANCE.setLog( "Error in purchase", e.getMessage().toString());
								   ...
								   final String oldProductId)
	{
		...
		CommonUtils.INSTANCE.setLog(LOG_TAG, "startPurchaseFlow called productId:"+productId+"\noldProductId:"+oldProductId +"\nskuDetails:"+skuDetails);
		...
		handler.post(new Runnable()
		{
			...
			public void run()
			{
				...
				String offerToken = "";
				 if(skuDetails.getSubscriptionOfferDetails() !=null && skuDetails.getSubscriptionOfferDetails().size()>0)
					 offerToken= skuDetails
...
// in the billing flow creating separate productDetailsParamsList variable
				...
				if (!TextUtils.isEmpty(productId))
				{
					PurchaseInfo oldProductDetails = getSubscriptionPurchaseInfo(productId);
				...
				CommonUtils.INSTANCE.setLog(LOG_TAG, "startPurchaseFlow launchBillingFlow responseCode"+responseCode);
	...
	private void handleOwnedPurchaseTransaction(String productId)
	{
		...
		CommonUtils.INSTANCE.setLog(LOG_TAG, "handleOwnedPurchaseTransaction details:"+details);

		if (!checkMerchant(details))
		{
			CommonUtils.INSTANCE.setLog(LOG_TAG, "Invalid or tampered merchant id!");
	...
	public void consumePurchaseAsync(final String productId, final IPurchasesResponseListener listener)
	{
				...
				billingService.consumeAsync(consumeParams, new ConsumeResponseListener()
				{
												  ...
												  @NonNull String purchaseToken)
					{
						if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK)
						{
							...
							CommonUtils.INSTANCE.setLog(LOG_TAG, "Successfully consumed " + productId + " purchase.");

							reportPurchasesSuccess(listener);
						}
						else
						{
							CommonUtils.INSTANCE.setLog(LOG_TAG, "Failure consume " + productId + " purchase.");
		...
		{
			CommonUtils.INSTANCE.setLog("Error in consumePurchase", e.getMessage().toString());
...
//																	.build();
				...
				billingService.queryProductDetailsAsync(
						...
						{
							@Override
							public void onProductDetailsResponse(@NonNull BillingResult billingResult, @NonNull List<ProductDetails> detailsList) {
								int response = billingResult.getResponseCode();
								if (response == BillingClient.BillingResponseCode.OK)
								{
									if (detailsList != null && detailsList.size() > 0)
									{
										for (ProductDetails skuDetails : detailsList)
										{
											//												JSONObject object =
//														new JSONObject(skuDetails);
								...
								else
								{
									...
									CommonUtils.INSTANCE.setLog(LOG_TAG,
											String.format(
			...
			{
				CommonUtils.INSTANCE.setLog( "Failed to call getSkuDetails", e.getMessage().toString());
	...
	private void verifyAndCachePurchase(Purchase purchase)
	{
		...
		try
		{
			...
			{
				CommonUtils.INSTANCE.setLog(LOG_TAG, "Public key signature doesn't match!");
				reportBillingError(Constants.BILLING_ERROR_INVALID_SIGNATURE, null);
			}
		}
		catch (Exception e)
		{
			CommonUtils.INSTANCE.setLog("Error in handleActivityResult", e.getMessage().toString());
			reportBillingError(Constants.BILLING_ERROR_OTHER_ERROR, e);
		}
		savePurchasePayload(null);
	}
