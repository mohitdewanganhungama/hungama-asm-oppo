...
import com.hungama.music.databinding.LayoutBodyPowerSpinnerLibraryBinding
...
class PowerSpinnerView : AppCompatTextView, LifecycleObserver {
  ...
  init {
    ...
    }else if (lifecycleOwner == null && viewContext is LifecycleOwner) {
      lifecycleOwner = viewContext
    }

  }

  var spinnerObserver:SpinnerObserver? = null
  ...
  fun show(xOff: Int = 0, yOff: Int = 0) {
    try {
      debounceShowOrDismiss {
        if (!isShowing) {
          if (spinnerObserver != null){
            spinnerObserver?.onSpinnerObserver(true)
          }
          this.isShowing = true
          animateArrow(true)
  ...
  fun dismiss() {
    debounceShowOrDismiss {
      if (this.isShowing) {
        if (spinnerObserver != null){
          spinnerObserver?.onSpinnerObserver(false)
        }
        animateArrow(false)
  ...
  fun attachSpinnerObserver(spinnerObserver: SpinnerObserver) {
    this.spinnerObserver = spinnerObserver
  }
}
