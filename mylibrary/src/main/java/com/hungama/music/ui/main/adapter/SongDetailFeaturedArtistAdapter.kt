 ...
 */
class SongDetailFeaturedArtistAdapter(
    val context: Context,
    var arrayList:ArrayList<BodyRowsItemsItem?>?,
    val onitemclick: OnItemClick?
) :
    RecyclerView.Adapter<SongDetailFeaturedArtistAdapter.ItemViewHolder>() {
...
//        holder.bind(holidayList.get(pos))
        ...
        if (arrayList?.get(holder.adapterPosition)?.data?.image != null) {
            CommonUtils.setArtImageBg(true, arrayList?.get(holder.adapterPosition)?.data?.image!!, holder.binding.rootParent)
