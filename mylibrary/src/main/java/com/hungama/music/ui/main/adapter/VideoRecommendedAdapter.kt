...
class VideoRecommendedAdapter(
    val context: Context,
    var arrayList: ArrayList<BodyRowsItemsItem?>?,
    val onitemclick: OnItemClick?
) :
    RecyclerView.Adapter<VideoRecommendedAdapter.ItemViewHolder>() {
...
//            var language = arrayList?.get(holder.adapterPosition)?.data?.misc?.lang!!.joinToString  (separator = "|") {it}
...
//            holder.binding.tvLanguageView.text =
//                language + " • " + arrayList?.get(holder.adapterPosition)?.data?.misc?.f_playcount + " " + context.getString(R.string.discover_str_25)
