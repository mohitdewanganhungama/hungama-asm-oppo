...
import com.hungama.music.databinding.RowArtistUpcomingBinding
 ...
 */
class ArtistUpcomingAdapter(
    val context: Context,
    var arrayList: List<BodyRowsItemsItem?>?,
    val onitemclick: OnItemClick?
) :
    RecyclerView.Adapter<ArtistUpcomingAdapter.ItemViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
        val binding: RowArtistUpcomingBinding = DataBindingUtil.inflate(
            LayoutInflater.from(parent.context),
            R.layout.row_artist_upcoming,
            parent,
            false
        )
    ...
    class ItemViewHolder(val binding: RowArtistUpcomingBinding) :
...
//        holder.bind(holidayList.get(pos))
        ...
        if (arrayList?.get(holder.adapterPosition)?.data?.misc != null) {
            holder.binding.tvBtnPrice.text = "₹"+arrayList?.get(holder.adapterPosition)?.data?.misc?.favCount
            holder.binding.tvBtnPrice.visibility = View.VISIBLE
        } else {
            holder.binding.tvBtnPrice.visibility = View.GONE
        }
