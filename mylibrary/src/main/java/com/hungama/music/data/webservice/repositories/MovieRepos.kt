...
class MovieRepos {
    ...
    fun getMovieDetail(context: Context, id: String): MutableLiveData<Resource<PlaylistDynamicModel>> {
        CoroutineScope(Dispatchers.IO).launch{
            val url = WSConstants.METHOD_PLAYABLE_CONTENT_DYNAMIC + id + "/movie/detail"
            setLog(TAG, "getMovieDetail: url:$url")
            ...
            DataManager.getInstance(context)?.getVolleyRequest(context,url, JSONObject(),object : DataValues {
                override fun setJsonDataResponse(response: JSONObject?) {
                    ...
                    try {
                        val dataModel = Gson().fromJson<PlaylistDynamicModel>(
                            response.toString(),
                        ...
                        hashMap.put(EventConstant.NAME_EPROPERTY, "movie_detailsscreen")
                        hashMap.put(
