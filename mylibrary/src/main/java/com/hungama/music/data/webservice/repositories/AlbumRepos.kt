...
class AlbumRepos {
    ...
    private var albumResp = MutableLiveData<Resource<PlaylistDynamicModel>>()
    fun getAlbumDetailList(context: Context, id: String): MutableLiveData<Resource<PlaylistDynamicModel>> {

        CoroutineScope(Dispatchers.IO).launch{
            val url= WSConstants.METHOD_PLAYABLE_CONTENT_DYNAMIC+id+"/album/detail"
            ...
            DataManager.getInstance(context)?.getVolleyRequest(context,url, JSONObject(),object : DataValues {
                override fun setJsonDataResponse(response: JSONObject?) {
                    if (albumResp == null) {
                        albumResp = MutableLiveData<Resource<PlaylistDynamicModel>>()
                    }

                    try {
                        val albumModel = Gson().fromJson<PlaylistDynamicModel>(
                            response.toString(),
                        ...
                        hashMap.put(EventConstant.NAME_EPROPERTY, "album_details")
                        hashMap.put(
