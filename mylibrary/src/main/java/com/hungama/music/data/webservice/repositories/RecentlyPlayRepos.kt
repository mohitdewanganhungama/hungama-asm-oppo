...
class RecentlyPlayRepos {
    private val TAG = javaClass.simpleName
    private var continueWhereLeftResp = MutableLiveData<Resource<ContinueWhereLeftModel>>()
    ...
    fun getContinueWhereLeftList(context: Context, bottomTabID: Int, headItemsItem: HeadItemsItem?): MutableLiveData<Resource<ContinueWhereLeftModel>> {
        CoroutineScope(Dispatchers.IO).launch{
            val page = headItemsItem?.page
            var url = WSConstants.METHOD_USER_UPDATE_STREAM + SharedPrefHelper.getInstance()
                .getUserId() + "/stream"
            if (page.equals("movies", false)){
                url = WSConstants.METHOD_USER_UPDATE_STREAM + SharedPrefHelper.getInstance()
                    .getUserId() + "/stream/type?typeId=4,93"
            }else if (page.equals("tvshow", false)){
                url = WSConstants.METHOD_USER_UPDATE_STREAM + SharedPrefHelper.getInstance()
                    .getUserId() + "/stream/type?typeId=96,98"
            }else if (bottomTabID== Constant.WATCH_TAB){
                url = WSConstants.METHOD_USER_UPDATE_STREAM + SharedPrefHelper.getInstance()
                    .getUserId() + "/stream/type?typeId=4,93,96,98"
            }else if (bottomTabID== Constant.LISTEN_TAB){
                url = WSConstants.METHOD_USER_UPDATE_STREAM + SharedPrefHelper.getInstance()
                    .getUserId() + "/stream/type?typeId=21,55555,1,88888,22"
            }


            var requestTime = DateUtils.getCurrentDateTime()
            continueWhereLeftResp.postValue(Resource.loading(null))
            DataManager.getInstance(context)?.getVolleyRequestArray(context,url, object : DataValues {
                override fun setJsonArrayDataResponse(jsonArray: JSONArray?) {
                    try {
                        ...
                        continueWhereLeftResp.postValue(Resource.success(continueWhereLeftModel))
                        ...
                        hashMap.put(EventConstant.SOURCE_NAME_EPROPERTY, ""+HungamaMusicApp.getInstance().getEventData("stream").sourceName)
                        hashMap.put(EventConstant.SOURCE_EPROPERTY, ""+HungamaMusicApp.getInstance().getEventData("stream").sourceName)
                        ...
                        exp.printStackTrace()
                        continueWhereLeftResp.postValue(Resource.error(context.getString(R.string.discover_str_2), null))
                    }
                }
