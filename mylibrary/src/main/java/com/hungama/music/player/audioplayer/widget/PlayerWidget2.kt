...
class PlayerWidget2 : BaseWedget() {

    override fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager,
                                appWidgetId: Int) {

        val views = RemoteViews(context.packageName, R.layout.player_widget2)
        ...
        if (noData) {
            ...
            views.setTextViewText(R.id.widget_subTitle, mSubTitle)
            ...
            views.setOnClickPendingIntent(R.id.widget_next,
                        getPendingIntent(context, PLAYNEXT))
            views.setOnClickPendingIntent(R.id.widget_prev,
                        getPendingIntent(context, PLAYPREVIOUS))
